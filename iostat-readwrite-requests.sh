...
set ylabel "Requests"
set samples 10
plot "$1" using 1:4 title "read req merged per sec (rrqm/s)" with lines, \
"$1" using 1:5 title "write req merged per sec (wrqm/s)" with lines, \
"$1" using 1:6 title "read req per sec (r/s)" with lines, \
"$1" using 1:7 title "write req per sec (w/s)" with lines
_EOF_
